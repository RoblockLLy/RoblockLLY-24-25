...
* Fecha: 13/05/2024
* Descripci�n: LevelManager: Manager gen�rico de niveles, maneja las c�maras, los botones del canvas
...
public class LevelManager : MonoBehaviour {
    ...
    private List<Vector3> movableObjectsSpawnPositions = new List<Vector3>();       // Posici�n inicial a reiniciar de dichos objetos
    private List<Quaternion> movableObjectsSpawnRotations = new List<Quaternion>(); // Rotaci�n inicial a reiniciar de dichos objetos

    public GameObject errorPanel;           // Panel de errores
    public Text errorTitle;                 // T�tulo del panel de errores

    public List<GameObject> victoryPanels;  // Paneles de victoria de cada canvas a activar al lograr una victoria
    public List<Text> challengeTitles;      // T�tulos en los paneles de victoria a reemplazar con el nombre del desaf�o
    public List<Text> timeResults;          // Textos en los paneles de victoria a reemplazar con el tiempo logrado
    public List<Text> blockCountResults;    // Textos en los paneles de victoria a reemplazar con el n�mero de bloques usado
    public List<Text> scoreResults;         // Textos en los paneles de victoria a reemplazar con la calificaci�n obtenida
    ...
    public SettingsMenuManager settingsMenuManager; // Manager del men� de opciones para pedir una actualizaci�n de los botones de estad�sticas al ganar un nivel
    private StatsManager statsManager;              // Manager de estad�sticas para guardar los valores de un intento

    public List<GameObject> spawnPoints; // Lugares de aparici�n de robots
    public List<GameObject> flags;       // Banderas de victoria
    public List<GameObject> movableObjects; // Objetos a reiniciar posici�n, rotaci�n y cin�tica al inicio de cada intento
    private List<GameObject> robots = new List<GameObject>();   // Lista de robots

    public string levelName;        // Nombre del nivel para guardar en las estad�sticas de juego
    ...
    void Start() {  // Guardamos en una lista los robots para m�s f�cil acceso, luego los movemos a sus respectivos lugares de aparici�n
        ...
        // Desactivamos la cin�tica de los robots y objetos a reiniciar por intento
        DisableElements();
        // Guardamos la posici�n y rotaci�n de los objetos a reiniciar por intento
        ...
        // Obtenemos la referencia al manager de estad�sticas y le introducimos los valores del r�cord a batir (dev stats)
    ...
    void ShowError((string type, string position) message) { // Abre el panel de error con el error a ense�ar
    ...
    public void EnableElements() { // Esta funci�n activa la cin�tica de los robots y objetos a reiniciar por intento
    ...
    public void DisableElements() { // Esta funci�n desactiva la cin�tica de los robots y objetos a reiniciar por intento
        foreach (GameObject robot in robots) {
    ...
    public void ResetChrono() { // Pausa y reinicia el contador de tiempo (el texto no, puede a�adirse si se desea)
        chronoRunning = false;
        chronoTime = 0.0f;
    ...
    public void ResetLevel() { // Reinicia el nivel por completo
        // Paramos el int�rprete de ublockly si estuviese ejecut�ndose
        UBlockly.CSharp.Runner.Stop();
        // Desactivamos la cin�tica de robots y objetos a reiniciar por intento
        DisableElements();
        // Movemos al punto de aparici�n robots y objetos
        for (int i = 0; i < spawnPoints.Count; i++) {
            robots[i].transform.position = spawnPoints[i].transform.position;
            robots[i].transform.rotation = spawnPoints[i].transform.rotation;
        }
        ...
        // Reiniciamos el contador de tiempo
        ResetChrono();
        // Limpiamos las se�ales enviadas
        CommsManager.commsManager.RemoveAllSignals();
        // Limpiamos las agrupaciones de robots
        CommsManager.commsManager.ClearGroups();
        // Activamos los scripts de las banderas por si est�n desactivados debido a una victoria
        foreach (GameObject flag in flags) {
    ...
    void Victory() { // En caso de victoria
        // Paramos al int�rprete de ublockly si estuviese en ejecuci�n y desactivamos la cin�tica de robots y objetos
        UBlockly.CSharp.Runner.Stop();
        DisableElements();
        // Si la victoria se obtuvo en ejecuci�n normal y no con debug para evitar hacer trampas:
        if (!controlPanel.getDebugMode()) {
            // Desactivamos el contador de tiempo
            chronoRunning = false;
            // Obtenemos las estad�sticas del nivel y subimos al manager de estad�sticas las obtenidas en este intento
            ...
            // Llamamos al manager del men� de opciones para avisarle de que tiene que actualizar los botones de stats por nivel
    ...
    public void Improve() { // Esta funci�n se llama al pulsar el bot�n de mejorar intento en el panel de victoria
    ...
    string CalculateScore(Stats devStats) { // Calcula las puntuaciones/calificaci�n de un intento teniendo en cuenta los valores a vencer
        int min = (int)(chronoTime / 60);
        int sec = (int)(chronoTime % 60);
        ...
        // C si el tiempo es mayor que el del dev por menos de 10 segundos y usa m�s bloques
        if (extraSeconds <= 10) {
            return "C";
        }
        // C si el tiempo es mayor que el del dev por menos de 30 segundos y usa m�s bloques
        if (extraSeconds <= 30) {
            return "D";
        }
        // F si el tiempo es mayor que el del dev por m�s de 30 segundos y usa m�s bloques
        return "F";
    }

    public void ExitLevel() { // Vuelve al men� de selecci�n de niveles
